...
mv $WORKSPACE/B2G/upload/sources.xml $HOME/artifacts-public/sources.xml
...
mv $WORKSPACE/B2G/upload/gaia.zip $HOME/artifacts-public/gaia.zip
...
if [ -f $WORKSPACE/B2G/upload/fota-*-update-*.mar ]; then
  if [ "${TARGET}" = "nexus-4-kk" -o "${TARGET}" = "nexus-5-l" ]; then
    mv $WORKSPACE/B2G/upload/fota-*-update-*.mar $HOME/artifacts-public/
  else
